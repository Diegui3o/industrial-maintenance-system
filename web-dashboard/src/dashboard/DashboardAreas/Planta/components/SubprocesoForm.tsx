...
export function SubprocesoForm({
  ...
  const cargarProcesos = async () => {
    ...
    try {
      ...
      if (!procesoId && resultado.length === 1) {
        setProcesoId(resultado[0].id.toString());
      }
    } catch (error) {
    ...
    <div className="planta-form">
      {loadingProcesos ? (
        ...
        <div className="planta-alert warning">
          ...
          <span>
            Primero debe crear un proceso
            antes de crear un subproceso.
          </span>
        </div>
      ) : (
        <>
